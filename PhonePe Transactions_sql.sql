...
WITH brand_engagement AS (SELECT State, Brand, Year, Quarter, SUM(Registered_Users) AS total_registered_users, SUM(App_Opens) AS total_app_opens,
...
growth_analysis AS (SELECT *, ((total_app_opens - prev_app_opens) / NULLIF(prev_app_opens, 0)) * 100 AS growth_percentage
